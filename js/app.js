...
function handleCurrencySelection() {
  $('.js-currency-btn').click(function(event) {
    if (!$(this).hasClass('is-outlined')) {
      ...
      $('.js-currency-btn.is-outlined').removeClass('is-warning is-outlined');
      // Select new currency
...
function handleRangeSelection() {
  ...
  $('.js-range-btn').click(function(event) {
    if (!$(this).hasClass('is-outlined')) {
      ...
      $('.js-range-btn.is-outlined').removeClass('is-warning is-outlined');
...
function getBaseChartOptions(latestPrice) {
  ...
  let chartOptions = {
    chart: {
      ...
      description: `${STORE.scale} price history chart for ${STORE.coin}, denominated in ${
        STORE.tsym
      }`
    },
    navigator: {
...
function addRangeChartOptions(chartOptions) {
  ...
  if (STORE.range === '1d') {
    ...
    tooltipHeaderFormat = '<span style="font-size: 10px">{point.x:%A, %b %d, %Y}</span><br/>';
